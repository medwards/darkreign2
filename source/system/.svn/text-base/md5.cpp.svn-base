...
namespace MD5
{
  ...
  static void Process(State &state, const U8 *data /*[64]*/)
  {
    U32 a = state.abcd[0], b = state.abcd[1], c = state.abcd[2], d = state.abcd[3];
    U32 t;

    U32 xbuf[16];
    const U32 *X;

    if (!((data - (const U8 *) 0) & 3)) 
    {
  ...
  void Init(State &state)
  {
    ...
    state.abcd[1] = 0xefcdab89;
    state.abcd[2] = 0x98badcfe;
  ...
  void Append(State &state, const U8 *data, U32 size)
  {
    ...
    if (offset) 
    {
	    int copy = (offset + size > 64 ? 64 - offset : size);
  ...
  void Finish(State &state, U8 digest[16])
  {
    ...
    for (i = 0; i < 8; ++i)
    {
  	  data[i] = (U8)(state.count[i >> 2] >> ((i & 3) << 3));
    }
    ...
    for (i = 0; i < 16; ++i)
    {
	    digest[i] = (U8)(state.abcd[i >> 2] >> ((i & 3) << 3));
    }
  }
}
